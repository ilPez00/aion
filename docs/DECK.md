...
Uno, two thumb joysticks, a clickable rotary wheel, four face buttons, a mode
...
- **Wheel (KY-040)** — the highest-value single control for a list-based TUI:
  detented scroll + click-to-run is one finger, eyes-free. It sits on D2/D3,
  the Uno's only external-interrupt pins, so no detent is ever missed.
...
| Control | Intent |
|---------|--------|
| wheel ± | navigate up/down |
| wheel click / joy2 click | activate (run harness / pause task) |
...
| wheel ± | REL_WHEEL (system scroll) |
| A / B / X / Y | BTN_SOUTH / EAST / WEST / NORTH |
| joy2 click / wheel click | BTN_THUMBL / BTN_SELECT |
...
in order of effort: second wheel on pin-change interrupts (A2/A3 spare ADC
gone though), an I/O expander (MCP23017) for more buttons on the same I2C
bus, or port the same headers to a Mega/ESP32 — `deck.h`/`joynav.h` are pure
logic and compile anywhere.
