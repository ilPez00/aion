...
SAMPLE_PAYLOAD = {
    ...
    "counts": {"memory": 2, "sessions": 3, "docs": 2},
}
...
def db_path(tmp_path):
    p = tmp_path / "agg.db"
    # point the module default at the tmp file for lookup helpers
...
def test_ingest_node_payload_populates_db(db_path):
    ...
    assert r["ingested"]["total"] == 7
    st = agg.status(db_path)
    assert st["exists"] is True
    assert st["items"] == 7
    assert st["by_kind"] == {"memory": 2, "session": 3, "doc": 2}
    assert st["by_node"] == {"omo": 7}
...
def test_ingest_is_idempotent_upsert(db_path):
    ...
    agg.ingest_node_payload(db_path, SAMPLE_PAYLOAD)
    st = agg.status(db_path)
    # upsert by (kind,node,source,ref) — re-run must not duplicate rows
    assert st["items"] == 7
...
def test_collect_all_fake_transport_aggregates_two_nodes(db_path):
    ...
    st = r["summary"]
    assert st["by_node"] == {"omo": 7, "pansa": 7}
    assert st["items"] == 14
...
def test_collector_source_is_loadable():
    ...
    assert "print(json.dumps" in src or "print(json.dumps" in src.replace("\n", " ")
...
def test_render_mesh_shows_aggregate():
    ...
    data = {
        ...
        "agg": {
            "exists": True, "items": 12,
            "by_node": {"omo": 7, "pansa": 5},
            "by_kind": {"session": 8, "memory": 4},
            "recent": True,
        },
    }
